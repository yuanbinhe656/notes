...
2. 服务器
   ...
   - fastCGI
     - 帮助服务器处理动态请求
   - eg：常见服务器架构：ngnix做反向代理，通过集群，动态请求转发给集群。MySQL中存入文件信息，但并不存文件内容，文件内容通过专门的文件服务器来进行存储，使用kv对，通过redis来进行MySQL缓存
3. 反向代理服务器
...
>    - 不需要在同一地点
> 3. 需要管理者
>    1. 管理者主要管理主机，发给管理者一个文件，管理者告诉请求方文件所在的主机，请求方在迭代进行查询
> 4. 编写应用层的管理程序
>    - 不需要编写
>    - 不需要重复造轮子，有大量可用的代码框架
...
2. fastDFS框架中的三个角色
   ...
   - 存储节点 - storage - 守护进程
     - 存储节点是有多个的
       - 理论无上限
     - 不需要和用户进行直接交互，通过终端交互，只有用户请求到达才进行唤醒
      ...
      - 最后启动，前两者准备工作
        - 上传
          - 连接追踪器, 询问存储节点的信息
            ...
            - 通过得到IP和端口连接存储节点
            - 将文件内容发送给存储节点
            - 存储节点将文件id返回给客户端，客户端通过文件id请求
              - 若是客户端通过文件名进行访问，则需要存储器管理文件名，可能有不同用户重名文件，还需要通过客户端加文件名进行存储。增加管理成本
        - 下载
          - 连接追踪器, 询问存储节点的信息
            - 问一下, 要下载的文件在哪一个存储节点
            - 追踪器查询, 得到结果
...
4. fastDFS集群 - （了解即可）
   ...
   2. 存储节点集群
      - fastDFS管理存储节点的方式？
        - 通过分组的方式完成的
        - 多个存储节点属于一个组，一组一般都是存储的同一个数据，属于纵向扩容
      - 集群方式（扩容方式）
        - 横向扩容 - 增加容量
          - 添加一台新的主机 （前提增加新的组）-> 容量增加了
...
1. tracker 配置文件
   ...
   # 追踪器监听的端口
   port=22122
   # 追踪器存储日志信息的目录, xxx.pid文件, 必须是一个存在的目录，防止守护进程出现问题
...
- group1
  ...
  - 如果有多个组这个组名可变的
- M00 - 虚拟目录
  - 和存储节点的配置项有映射   ==路径是可变的，因此使用变量进行存储==
