...
3. location表达式类型  
   ...
   - ~* 表示执行一个正则匹配，不区分大小写
   - ^~  表示普通字符匹配。使用前缀匹配。如果匹配成功，则不再匹配其他location。==式子不按照正则表达式语法解读==eg：location ^~ .jpg 若是~* ，则匹配第一个字符为任意字符其后为jpg的，在本情况中，其表示前缀为.jpg的式子
   - = 进行普通字符精确匹配。也就是完全匹配。==请求的内容就是式子的内容，不能式子是请求的前缀==

4. 匹配模式及优先级顺序(高 -> 低): 等号最高  优先匹配非正则 跟在代码中的==位置无关==  ==带修饰的表达式比所有不带修饰的优先度都高==
    ...
    | location /uri                                 | 不带任何修饰符，也表示前缀匹配，但是在正则匹配之后。优先匹配前缀长的式子 |
...
>
>     ==秘钥是在客户端浏览器生成的，并且必须在客户端浏览器生成，因为只有公钥加密才有效，而你只有百度的公钥，百度没有你的公钥之后将使用对称加密算法进行通信==
...
>    - 公钥加密, 私钥解密
>      - 数据传输的时候使用
>    - 私钥加密, 公钥解密  数据传输时无意义，因为不安全，任可人都可以进行解密
>      - 数字签名
>
> 3. 哈希函数
>
...
>      - 验证数据的一致性型
>    - 弊端:
>      - 两端共享秘钥必须相同, 共享秘钥分发困难 P2P难分发，需要一个中间人
>
...
>      - 得到的密文就是数字签名
>
>    - 签名的校验:==加密的并非整个文件，而是对文件的散列值进行加密==   侧重并非文件加密，而是对双方交流人的确认
>
>      - 校验这会收到签名者发送的数据
>        - 原始数据
>        - 数字签名
...
>        - 相同 -> 校验成功了, 数据属于签名的人
>        - 失败 -> 数据不属于签名的人
>      - 可以第三方校验，任何有公钥的人都有对其进行检验
>
>    - 弊端:
>
>      - 接收公钥的人没有办法校验公钥的所有者，比如希望跟A通信，结果B中间篡改了，但是不知道A的公钥是不是这个，需要一个中间人，发放A的公钥，加上A的个人信息，这些信息通过中间人的私钥进行解密。相当于必须有一个信任的中间人
>
