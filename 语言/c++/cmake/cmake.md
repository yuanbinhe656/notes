...
> 在C++中可以通过输出__cplusplus查看当前编译器所用的标准，.c文件中
> 
...
提供一个布尔变量，可以让用户自行选择，一般在源文件中来进行不同情况的判断
...
> 1. 当变量为0时，cmakedefine不会进行定义
> 2. 当变量为option，且为off时，其不会定义，为on时才会定义
> 3. 定义一个未定义的变量，其不会定义
...
**PUBLIC**：本目标需要，其他文件也需要

**PRIVATE**：本目标需要，其他文件不需要

**INTERFACE**：本目标不需要，其他文件需要
