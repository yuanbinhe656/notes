...
1. +
    1. 只有两边同时为数字类型时才进行相加运算，否则均进行字符串
...
​	==若同时使用var fn = function fc{}()==，则显示fc未定义，只能使用fn进行调用
...
```
1. 自调用函数和匿名函数不同，自调用函数不能有返回值，无法获取
...
- 设置延迟函数
    ...
    - 延迟函数只能使用匿名函数，不可以使用函数再调用，同时，在进行取消延迟函数时，也必须使用
         ...
         [⪹㿂鸝ྣ蟉㿊㒔◴ܒ耀](D:\soft\Typora\⪹㿂鸝ྣ蟉㿊㒔◴ܒ耀)  [途易跢](D:\soft\Typora\途易跢)  [﻿￪쳿ꘀ�"秿ࠀ￸P](D:\soft\Typora\﻿￪쳿ꘀ�"秿ࠀ￸P)  [ｮÛＡ⛿](D:\soft\Typora\ｮÛＡ⛿)  [㐀￿ᐓ￿K](D:\soft\Typora\㐀￿ᐓ￿K)  [ｇ⯿쨀틿턝￿2老﻿﷧￡⓿](D:\soft\Typora\ｇ⯿쨀틿턝￿2老﻿﷧￡⓿)  [뤬㶥ﳿ
        ](D:\soft\Typora\뤬㶥ﳿ
        ) 
...
- 数组常用方法
  ...
  1. splice()	//删除或替换当前数组的某些项目，参数start, deleteCount, options(要替换的项目)
  
      1. 当deletCount=1时，后面有option，则进行替换
      2. 当deletCount=0时，后面有option，则进行插入
  
  2. map
  
      1. 对元素进行操作并返回，与foreache一样，有返回值，返回值再组成一个array
...
1. 两种格式
    ...
    2. new关键字声明
        ...
        1. ```
            var reg = new RegExp("表达式主体","修饰符");
            例如：
            var reg = new RegExp("white","g"); // new 关键字声明
            ```
    ...
    4. 修饰符表示属性
        ...
        3. m 多行匹配
            ...
            1. m一般和行首\^和行尾\&结合使用，在行中位置的限定，由于一个字符串虽然显示可能多行，但实际只有一行，因此只有一个\^和一个\&,因此只会匹配字符串第一行或者最后一行，使用m后，会将显示的每一行都加上行首和行尾符，则其可以针对显示每一行进行操作

            2. m要与g一起合作，否则只能匹配第一行
            ...
            4. ```js
                    var strr1="abc";
                     var strr2="aaa";
                     sttr=strr1+"\n"+strr2;
                     var str1=sttr.replace(/^a/g,"#"); \\ #bc aaa
                     var str2=sttr.replace(/^a/m,"#"); \\ #bc aaa
                     var str3=sttr.replace(/^a/gm,"#"); \\ #bc #aa
                     var str4=sttr.replace(/a/gm,"#"); \\  #bc ###
                     console.log(sttr); 
                     console.log(str1);
                     console.log(str2);
                     console.log(str3);
...
2. 正则表示式常用字符
    ...
    1. 元字符  小写表示常规意思，大写表示相反意思
        1.  *.*匹配除了换行符之外的单个字符 
        2. \w 匹配一个数字、下画线或字母字符，等价于[A-Za-z0-9]  word单词
        3. \W 匹配任何非单字字符，等价于[^a-zA-z0-9] 
        4. \d 匹配一个数字字符，等价于[0-9]  data数字
        5. \D 匹配除数字之外的任何字符，等价于[^0-9] 
        6. \s 匹配任何空白字符   space空格
        7. \S 匹配任何非空白字符 
        8. \n 匹配换行符
    2. 特殊字符 对*需要使用 / 进行转义
        1. ^ 匹配字符串的开始 
        2. $ 匹配字符串的结束
        3.  | 匹配选择字符中任意一个，如 x|y，可匹配 x 或 y 
        4. () 分组 
        5. [] 匹配方括号内的任意一个字符 
        6. [^] 匹配不在方括号内的字符 
        7. {x} 匹配前一项 x 次
        8. {x,y} 匹配前一项至少 x 次，但是不能超过 y 次 
        9. *匹配前一项 0 次或多次，等价于{0,} 
        10. +匹配前一项 1 次或多次，等价于{1,} 
        11. ? 匹配前一项 0 次或 1 次，等价于{0,1}  表示不确定是否存在
        12. ==总结==
            1. ^ 两种含义
                1. 开头
                2. 相反
