...
2. 内部样式表
    ...
    2. 一般写在head中，不写在body中,进行了分离，但分离不彻底
...
（2）==高、宽无效==，但水平方向的padding和margin可以设置，垂直方向的无效。可以将其display转化为块元素，将其修改高度和宽度
...
2、box-sizing: border-box  盒子大小为 width    就是说  padding 和 border 是包含到width里面的，若设置了padding，则content内容自动为width-padding
...
1）Visible（默认值）：内容不会被修剪，会呈现在元素 图 8-12 内边距设置方式 第 8 章 CSS 盒模型与浮动定位 111 框之外。 
...
| -------- | :----------------------------------------------------------- |
| static   | 自动定位（默认定位方式）                                     |
| relative | 相对定位，相对于其原文档流的位置进行定位==其还占原文档流位置，只是显示变了== |
...
相对定位是将元素==相对于它在标准流中的位置==进行定位，当position属性的取值为relative时，可以将元素定位于相对位置。
...
给要定位的元素加个父块，父块位置设置为相对定位，让子元素实现绝对定位



## 绝对定位的盒子水平/垂直居中

普通的盒子是左右margin 改为 auto就可， 但是对于绝对定位就无效了==水平可以使用margin：0 auto进行居中，而不能使用auto对其进行垂直居中，因为高度是由内容撑起来的==
...
### 轮廓 outline
 ...
 是绘制于元素周围的一条线，位于边框边缘的外围，可起到突出元素的作用，相当于周围加阴影，影响显示效果。
