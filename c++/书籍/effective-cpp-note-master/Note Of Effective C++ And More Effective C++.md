...
实际是：应该让编译器代替预处理器定义，因为预处理器定义的变量并没有进入到symbol table（记号表，经过预编译器处理后提交给编译器的参数表）里面。编译器有时候会看不到预处理器定义

缺点：

1. 当在同一个文件中使用宏时，无法有效用编译器进行追踪错误

2. 当不在同一个文件夹使用宏时，更加难以追踪错误

3. 可能会导致编译后的代码出现多次该宏所代表的数字而非其变量
...
来代替 
    ==一般常量==均存储在头文件中，以便被用户所访问，对于常量指针来说，不仅其指针所指对象不能改变，该指针也不能指向其他对象，因此必须声明为指向常量的常指针
