...
`capacity`函数返回容器目前最多可以容纳的==元素数量==。

`reserve`函数告知容器应该准备保存多少元素，它并不改变容器中元素的数量，仅影响容器预先分配的内存空间大小。一般容器预存储容器大小是当前capacity的2倍，每次capacity满时，其才会重新分配一块内存，并将该vector的内容remove过去
